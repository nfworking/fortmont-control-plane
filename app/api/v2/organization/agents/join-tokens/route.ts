...
import { generateJoinToken, hashJoinToken, jsonError, requireDashboardSession } from "@/lib/server/agents";
...
export async function GET() {
  ...
  const tokens = await db
    ...
    .from(agentJoinToken)
    .where(and(eq(agentJoinToken.organizationId, orgContext.activeOrganization.id), eq(agentJoinToken.revoked, false), gt(agentJoinToken.expiresAt, now)))
...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json({ token: joinToken, tokenInfo: created }, { status: 201 });
}
...
export async function DELETE(request: NextRequest) {
  ...
  const [revoked] = await db
    ...
    .where(and(eq(agentJoinToken.id, tokenId), eq(agentJoinToken.organizationId, orgContext.activeOrganization.id)))
